...
MOD_DIR = os.path.join(PROJECT_ROOT, "mod_1_11_no")  # 运行前请确认文件夹名
...
def eval_loop():
    ...
    for t_idx in range(1, 11):
        ...
        with torch.no_grad():
            for w, v, y, _ in loader:
                ...
                # 先还原到 Log 域，再通过 exp 还原到物理域
                out_log = out_norm * (y_log_max - y_log_min + 1e-8) + y_log_min
                true_log = true_norm * (y_log_max - y_log_min + 1e-8) + y_log_min

                out_phys = np.exp(out_log)
                true_phys = np.exp(true_log)
    ...
    def draw_complex_plot(ax, data, labels, title, color):
        ...
        if plot_data:
            ax.boxplot(plot_data, positions=plot_positions, widths=0.15, showfliers=False,
    ...
    # 🌟 修改点 4：更新图表标题
    draw_complex_plot(ax1, r_plot_data, r_labels, '电阻 (R) 参数评估误差 - 11通道对齐汇总', 'steelblue')
    draw_complex_plot(ax2, c_plot_data, c_labels, '电容 (C) 参数评估误差 - 11通道对齐汇总', 'darkorange')
