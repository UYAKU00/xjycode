...
class TwinFusionNet(nn.Module):
    def __init__(self, n_channels=17, n_features=2, n_outputs=44):
        ...
        self.wave_conv = nn.Sequential(
            ...
            nn.Conv1d(128, 256, kernel_size=3, stride=2, padding=1),       # 128 -> 256
            nn.BatchNorm1d(256),
            nn.ReLU(),
        ...
        self.shared_fc = nn.Sequential(
            nn.Linear(320, 256),  # 之前是 192，现在升级为 320
            nn.ReLU(),
            nn.Dropout(0.2)
        )
...
if __name__ == "__main__":
    ...
    # 模拟外部调用方式
    model = TwinFusionNet(n_channels=17, n_features=2, n_outputs=44).to(device)
    print(f"✅ 模型已加载至: {device}")

    test_w = torch.randn(2, 17, 250).to(device)
